...
st.markdown("""
    ...
    .tm-lnk { display: inline-block; padding: 6px 12px; background: #1a3151; color: gold !important; text-decoration: none; border-radius: 5px; font-weight: bold; font-size: 12px; border: 1px solid gold; margin-top: 10px; }
...
if st.session_state.user is None:
    ...
    with st.container():
        st.markdown(f'<div class="barrow-box"><h3>🕵️‍♂️ Barrow AI</h3>{st.session_state.user}, kime bakmıştın? (Örn: "19 yaş forvet", "İtalyan stoper")</div>', unsafe_allow_html=True)
        q = st.text_input("", placeholder="Komut yaz...", key="ai_q", label_visibility="collapsed").lower()
        if q:
            ai_df = df.copy()
            # Kesin Yaş Filtresi
            ...
            # Millet
            mil = {"türk": "Tur", "italyan": "Ita", "alman": "Ger", "fransız": "Fra", "ispanyol": "Spa", "brezilya": "Bra"}
            ...
            if st.button("🔄 Başka Oyuncular Öner"):
                st.session_state.seed = random.randint(0, 5000)
            
            tops = ai_df.sort_values(by="PA", ascending=False).head(40)
            final = tops.sample(min(len(tops), 3), random_state=st.session_state.seed)
            
            c_ai = st.columns(3)
            for i, (idx, r) in enumerate(final.iterrows()):
    ...
    with tabs[0]: # SCOUT
        ...
        for idx, row in res.head(15).iterrows():
            ...
            color = "#FF4B4B" if perc < 45 else "#00FFC2"
            st.markdown(f"""<div class="p-card"><b>{row['Oyuncu']}</b> ({row['Yaş']})<br><small>{row['Kulüp']} | {row['Mevki']}</small><div class="progress-bar"><div class="progress-val" style="width:{perc}%; background:{color};"></div></div><small>PA: {row['PA']} | CA: {row['CA']} | 💰 {row['Değer']}</small>""", unsafe_allow_html=True)
    ...
    with tabs[2]: # LİSTE (BAĞLANDI ✅)
        ...
        if favs.data:
            ...
            st.dataframe(df[df['Oyuncu'].isin(f_names)][['Oyuncu', 'Yaş', 'PA', 'CA', 'Mevki', 'Değer']])
        else: st.warning("Henüz kimseyi eklemedin.")

    with tabs[3]: # KIYAS (BAĞLANDI ✅)
        pl_all = sorted(df['Oyuncu'].tolist())
        sel1 = st.selectbox("1. Oyuncu", ["-"]+pl_all, key="cmp1"); sel2 = st.selectbox("2. Oyuncu", ["-"]+pl_all, key="cmp2")
        if sel1 != "-" and sel2 != "-":
            st.table(df[df['Oyuncu'].isin([sel1, sel2])].set_index('Oyuncu')[['PA','CA','Yaş','Değer']])

    with tabs[4]: # KADROM (BAĞLANDI ✅)
        st.subheader("⚽ Kadro ve Bütçe")
        my_budget = st.number_input("Transfer Bütçen:", value=150000000)
        p_sel = ["Boş"] + sorted(df['Oyuncu'].tolist())
        c_k = st.columns(3)
        p_st = c_k[1].selectbox("Santrafor", p_sel, key="k_st")
        p_lw = c_k[0].selectbox("Sol Kanat", p_sel, key="k_lw")
        p_rw = c_k[2].selectbox("Sağ Kanat", p_sel, key="k_rw")
        
        sel_df = df[df['Oyuncu'].isin([p_st, p_lw, p_rw])]
        if not sel_df.empty:
            total = sel_df['ValNum'].sum()
            st.info(f"📊 Harcanan: {total:,.0f} € | Kalan: {my_budget - total:,.0f} €")

    with tabs[5]: # ADMİN (BAĞLANDI ✅)
        if any(x in st.session_state.user.lower() for x in ["someku", "omer", "ramazan"]):
            ...
            if logs.data: st.dataframe(pd.DataFrame(logs.data).tail(50))
        else: st.error("Yetkin yok.")
