...
# --- YÜKLEME EKRANI MANTIĞI ---
if 'lottie_shown' not in st.session_state:
    st.markdown("""
        <div class="loader-container">
            <div class="magnifier">🕵️‍♂️🔍</div>
            <h2 style="color: #58a6ff; margin-top: 20px;">Someku Elite Veritabanı Hazırlanıyor...</h2>
            <p style="color: #8b949e;">470.000+ Oyuncu Verisi Taranıyor</p>
        </div>
    """, unsafe_allow_html=True)
    time.sleep(2.5)
...
if 'announcements' not in st.session_state: 
    st.session_state.announcements = "🔥 SOMEKU SCOUT V82 Yayında! | 🎰 Rulet sekmesinde 21 yaş altı yıldızları keşfet."
            ...
            with cols[i % 2]:
                st.markdown(f'''
                    <div class="player-card {'fav-active' if is_fav else ''}">
                        ...
                        <p style="color:#8b949e; margin:5px 0;">🏟️ {p["kulup"]} | 👟 {p["mevki"]}</p>
...
with tabs[3]:
    ...
    if st.session_state.favs:
        for f in st.session_state.favs:
            ...
            c1.markdown(f'<div class="player-card fav-active" style="padding:10px;"><b>{f["oyuncu_adi"]}</b> ({f["kulup"]})</div>', unsafe_allow_html=True)
            if c2.button("🗑️ Sil", key=f"d_{f['oyuncu_adi']}"):
...
with tabs[5]:
    if st.session_state.user == "someku":
        at1, at2 = st.tabs(["✏️ Veri Düzenle", "📢 Duyuru"])
        with at1:
            e_search = st.text_input("Oyuncu Ara:")
            if e_search:
                e_res = supabase.table("oyuncular").select("*").ilike("oyuncu_adi", f"%{e_search}%").limit(1).execute()
                if e_res.data:
                    n_pa = st.number_input("Yeni PA:", value=int(e_res.data[0]['pa']))
                    if st.button("Güncelle"):
                        supabase.table("oyuncular").update({"pa": n_pa}).eq("oyuncu_adi", e_res.data[0]['oyuncu_adi']).execute()
