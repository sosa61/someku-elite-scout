...
def get_announcement():
    try:
        ...
        return res.data[0]['duyuru'] if res.data else "🔥 SOMEKU SCOUT Beta Yayında!"
    except: return "🔥 SOMEKU SCOUT Beta Yayında!"
        ...
        if st.button("Giriş", use_container_width=True):
            if u_id == "someku" and u_pw == "28616128Ok":
                ...
                try:
                    ...
                    if res.data: st.session_state.user = res.data[0]['username']; st.session_state.fav_list = get_user_favs(u_id); st.rerun()
...
with tabs[0]:
    ...
    REGIONS = {
        "Hepsi": [],
        "Avrupa": ["Almanya", "Fransa", "İngiltere", "İtalya", "İspanya", "Hollanda", "Portekiz", "Belçika", "Türkiye"],
        "Güney Amerika": ["Brezilya", "Arjantin", "Uruguay", "Kolombiya", "Ekvador"],
        "Kuzey Amerika": ["ABD", "Meksika", "Kanada"],
        "Afrika": ["Nijerya", "Senegal", "Mısır", "Fildişi Sahili", "Gana", "Cezayir", "Fas"],
        "Asya": ["Japonya", "Güney Kore", "Suudi Arabistan", "İran", "Katar"],
    ...
    with f3: age_f = st.slider("🎂 Yaş:", 14, 50, (15, 25)); pa_f = st.slider("📊 Potansiyel (PA):", 0, 200, (140, 200))
    with f4: sort_f = st.selectbox("🔃 Sıralama:", ["Potansiyel (PA)", "Yetenek (CA)", "Yaş (Gençten)"])
    
    sort_map = {"Potansiyel (PA)": "pa", "Yetenek (CA)": "ca", "Yaş (Gençten)": "yas"}
    asc_map = {"Potansiyel (PA)": False, "Yetenek (CA)": False, "Yaş (Gençten)": True}
    ...
    res = query.order(sort_map[sort_f], desc=not asc_map[sort_f]).range(st.session_state.page*12, (st.session_state.page*12)+11).execute()
            ...
            with cols[i % 2]:
                st.markdown(f'''
                    <div class="player-card {"fav-active" if is_fav else ""}">
                        ...
                        <h3>{p["oyuncu_adi"]}</h3>
                        <p style="color:#8b949e; margin:5px 0;">📍 {p["ulke"]} | 🏟️ {p["kulup"]} | 👟 {p["mevki"]}</p>
                        <p style="font-size:0.9rem;"><b>CA:</b> {p["ca"]} | <b>Yaş:</b> {p["yas"]} | <b>Değer:</b> {p.get("deger", "Bilinmiyor")}</p>
...
with tabs[1]:
    ...
    if st.session_state.roulette_player:
        ...
        tm_url = f"https://www.transfermarkt.com.tr/schnellsuche/ergebnis/schnellsuche?query={urllib.parse.quote(p['oyuncu_adi'])}"
        st.markdown(f'''
            <div class="player-card fav-active" style="max-width: 500px; margin: auto;">
                <span class="pa-badge">PA: {p["pa"]}</span>
                <h2 style="color:#f2cc60;">🌟 ŞANSLI YILDIZ</h2>
                <h3>{p["oyuncu_adi"]}</h3>
                <p>📍 {p["ulke"]} | 🏟️ {p["kulup"]} | 👟 {p["mevki"]}<br><b>CA:</b> {p["ca"]} | <b>Yaş:</b> {p["yas"]} | <b>Değer:</b> {p.get("deger", "Bilinmiyor")}</p>
                <a href="{tm_url}" target="_blank" class="tm-link">Transfermarkt ➔</a>
            </div>
        ''', unsafe_allow_html=True)
        if st.button("⭐ Favoriye Ekle", key="roul_fav"):
            if p['oyuncu_adi'] not in st.session_state.fav_list: st.session_state.fav_list.append(p['oyuncu_adi']); st.rerun()

# --- 3. 11 KUR ---
with tabs[2]:
    st.subheader("📋 Taktik Tahtası")
    f_names = st.session_state.fav_list if st.session_state.fav_list else ["Boş"]
    st.markdown('<div class="section-header">🧤 KALECİ</div>', unsafe_allow_html=True); gk = st.selectbox("GK:", f_names)
    st.markdown('<div class="section-header">🛡️ DEFANS</div>', unsafe_allow_html=True)
    d_cols = st.columns(4); dl = d_cols[0].selectbox("DL:", f_names); dc1 = d_cols[1].selectbox("DC1:", f_names); dc2 = d_cols[2].selectbox("DC2:", f_names); dr = d_cols[3].selectbox("DR:", f_names)
    st.markdown('<div class="section-header">⚙️ ORTA SAHA</div>', unsafe_allow_html=True)
    m_cols = st.columns(3); ml = m_cols[0].selectbox("AML:", f_names); mc = m_cols[1].selectbox("MC:", f_names); mr = m_cols[2].selectbox("AMR:", f_names)
...
with tabs[3]:
    ...
    if st.session_state.fav_list:
        for f_name in st.session_state.fav_list:
            ...
            if c2.button("🗑️", key=f"del_{f_name}"): st.session_state.fav_list.remove(f_name); st.rerun()
...
with tabs[4]:
    ...
    with st.form("o_form"):
        o_t = st.selectbox("Konu:", ["Veri Hatası", "Tasarım", "Özellik İsteği"]); o_m = st.text_area("Mesaj:")
        if st.form_submit_button("Gönder"):
            try: supabase.table("oneriler").insert({"ad": st.session_state.user, "konu": o_t, "mesaj": o_m}).execute(); st.success("Admin'e iletildi!")
            except: st.error("Hata!")
...
with tabs[5]:
    if st.session_state.user == "someku":
        ...
        try: p_c = supabase.table("oyuncular").select("id", count="exact").execute().count; st.metric("Toplam Oyuncu", p_c)
        except: pass
        ...
        with adm1:
            ...
            if e_s:
                ...
                if e_r.data:
                    ...
                    if st.button("Güncelle"): supabase.table("oyuncular").update({"pa": n_p}).eq("oyuncu_adi", e_target).execute(); st.success("Güncellendi!")
        with adm2:
            n_msg = st.text_area("Yeni Duyuru:", value=get_announcement())
            if st.button("Kaydet"):
                try: supabase.table("sistem").update({"duyuru": n_msg}).eq("id", 1).execute(); st.rerun()
                except: st.error("Hata!")
        with adm3:
            try: u_l = supabase.table("users").select("*").execute(); st.table(pd.DataFrame(u_l.data))
            except: st.info("Hata!")
        with adm4:
            try: o_l = supabase.table("oneriler").select("*").execute(); st.table(pd.DataFrame(o_l.data))
            except: st.info("Hata!")
