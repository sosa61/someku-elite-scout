...
# --- BARROW AYARLARI ---
BARROW_INSULTS = ["Cebinde kuruş yok hala elit oyuncu peşindesin. Al şunu dua et.", "Taktik bilgin zayıf, bari oyuncun düzgün olsun.", "Yine mi geldin? İstediğin kriterde adam bulmak zor ama Barrow bulur.", "Bak buraya hıyarto, bütçene göre en mermisi budur."]
BARROW_KNOWLEDGE = {"messi": ["AM R", "ST", "AM C"], "mbappe": ["ST", "AM L"], "ronaldo": ["ST", "AM L"], "van dijk": ["D C"]}

def get_announcement():
    try:
        res = supabase.table("sistem").select("duyuru").eq("id", 1).execute()
        return res.data[0]['duyuru'] if res.data else "🔥 SOMEKU SCOUT V110 Yayında!"
    except: return "🔥 SOMEKU SCOUT V110 Yayında!"
...
with tabs[0]:
    ...
    with f2: reg_f = st.selectbox("🌍 Bölge Seç:", list(REG_TR.keys())); country_f = st.text_input("Uyruk Ara:")
...
with tabs[1]:
    ...
    if st.session_state.roulette_player:
        ...
        st.markdown(f'''<div class="player-card {"fav-active" if is_f else ""}">
            ...
            <p>🏟️ {p["kulup"]} | 👟 {p["mevki"]} | 🎂 Yaş: {p["yas"]} | 💰 {p.get("deger","-")}</p>
            <a href="{tm_url}" target="_blank" class="tm-link">Transfermarkt ➔</a></div>''', unsafe_allow_html=True)
        if st.button(f"{'⭐ Favoriden Çıkar' if is_f else '⭐ Favorilere Ekle'}", key="rfav"):
            if is_f: supabase.table("favoriler").delete().eq("kullanici_adi", st.session_state.user).eq("oyuncu_adi", p['oyuncu_adi']).execute(); st.session_state.fav_list.remove(p['oyuncu_adi'])
            else: supabase.table("favoriler").insert({"kullanici_adi": st.session_state.user, "oyuncu_adi": p['oyuncu_adi']}).execute(); st.session_state.fav_list.append(p['oyuncu_adi'])
            st.rerun()

# --- 3. 11 KUR (EKSİKSİZ MEVKİLER & İNDİRME) ---
with tabs[2]:
    st.subheader("📋 Elite 11 Kadronu Kur")
    ...
    gk = k1.selectbox("GK:", f_n); lb = k2.selectbox("LB:", f_n); cb1 = k3.selectbox("CB1:", f_n); cb2 = k4.selectbox("CB2:", f_n); rb = k5.selectbox("RB:", f_n)
    
    st.markdown('<div class="section-header">⚙️ ORTA SAHA</div>', unsafe_allow_html=True)
    m1, m2, m3, m4 = st.columns(4)
    dm = m1.selectbox("DM:", f_n); cm1 = m2.selectbox("CM1:", f_n); cm2 = m3.selectbox("CM2:", f_n); amc = m4.selectbox("AMC:", f_n)
    
    st.markdown('<div class="section-header">🎯 FORVET</div>', unsafe_allow_html=True)
    s1, s2, s3 = st.columns(3)
    lw = s1.selectbox("LW:", f_n); rw = s2.selectbox("RW:", f_n); st_p = s3.selectbox("ST:", f_n)
    
    kadro_txt = f"GK: {gk}\nLB: {lb} | CB: {cb1}-{cb2} | RB: {rb}\nDM: {dm} | CM: {cm1}-{cm2} | AMC: {amc}\nLW: {lw} | RW: {rw} | ST: {st_p}"
    st.download_button("📸 KADROYU LİSTE OLARAK İNDİR", kadro_txt, file_name="someku_kadro.txt")
...
with tabs[4]:
    ...
    b_in = st.text_input("Emir ver (Örn: '5m forvet', '10m bek'):", key="b_in")
    if st.button("BARROWA SOR"):
        if b_in:
            ...
            req_p = float(nums[0]) if nums and ("m" in b_in.lower()) else None
            bq = supabase.table("oyuncular").select("*").lte("yas", 23).gte("pa", 150)
            
            # Mevki Filtresi
            if "stoper" in b_in.lower(): bq = bq.ilike("mevki", "%D C%")
            elif "bek" in b_in.lower(): bq = bq.or_("mevki.ilike.%D L%,mevki.ilike.%D R%")
            ...
            if res_b.data:
                ...
                final_b = [x for x in res_b.data if get_v(x) <= (req_p if req_p else 999)]
                if final_b:
                    ...
                    st.markdown(f'''<div class="player-card" style="border-left-color:#ff0000; background:#000;">
                        ...
                        <a href="{tm_url}" target="_blank" class="tm-link">Transfermarkt ➔</a></div>''', unsafe_allow_html=True)
                else: st.error("Bütçene göre adam yok hıyarto!")
