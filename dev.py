...
# --- 1. SCOUT (V165 - RULET SENKRONİZASYONLU FAVORİLEME) ---
with tabs[0]:
    # Mevki ve Bölge Sözlükleri (Dokunma, Bunlar Stabil)
    ...
    with f1: name_f = st.text_input("👤 Oyuncu Ara:"); team_f = st.text_input("Takım:")
    ...
    with v2: pa_f = st.slider("📊 PA Aralığı:", 0, 200, (135, 200))
    ...
    if res.data:
        ...
        for i, p in enumerate(res.data):
            ...
            with cols[i%2]:
                ...
                st.markdown(f'''
                <div style="padding:15px; border-radius:10px; margin-bottom:10px; border:{card_border}; background:rgba(255,255,255,0.03); position:relative;">
                ...
                if st.button(btn_txt, key=f"scout_btn_{p['oyuncu_adi']}_{i}", use_container_width=True):
                    if is_fav:
                        ...
                        # İŞTE RULETTEKİ O TAM PAKET İNSERT:
